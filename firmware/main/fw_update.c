...
static SemaphoreHandle_t _manifest_update_mut;
static SemaphoreHandle_t _manifest_rcvd;
static SemaphoreHandle_t _download_complete;
...
static TickType_t ms2ticks(int32_t ms)
{
    return (ms < 0 ? portMAX_DELAY : (uint32_t) ms / portTICK_PERIOD_MS);
}
...
static enum golioth_status fw_update_handle_block(const uint8_t *block,
                                                  size_t block_size,
                                                  size_t offset,
                                                  size_t total_size)
{
                               ...
                               void *arg)
{
    ...
    {
        ctx->result = status;
        xSemaphoreGive(_download_complete);
    }
    else
    {
        ctx->retries++;
...
static enum golioth_status fw_change_image_and_reboot()
{
    ...
    if (err != ESP_OK)
    {
        GLTH_LOGE(TAG, "Firmware change boot image failed (%s)", esp_err_to_name(err));
        return GOLIOTH_ERR_FAIL;
    }
...
static void fw_update_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (pdTRUE != xSemaphoreTake(_manifest_rcvd, ms2ticks(manifest_timeout)))
            {
                GLTH_LOGI(TAG,
        ...
        if (GOLIOTH_OK == err)
        {
            xSemaphoreTake(_download_complete, portMAX_DELAY);
                                        ...
                                        const struct golioth_fw_update_config *config)
{
    ...
    _manifest_update_mut = xSemaphoreCreateMutex();       // never destroyed
    _manifest_rcvd = xSemaphoreCreateCounting(1, 0);      // never destroyed
    _download_complete = xSemaphoreCreateCounting(1, 0);  // never destroyed
