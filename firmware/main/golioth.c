...
static enum golioth_settings_status
on_new_frame_setting(const char *new_value, size_t new_value_len, void *arg) {
  if (NULL == arg)
  {
  ...
  if (new_value_len >= sizeof(ctx->uri)) {
    ESP_LOGE(TAG, "New string too long: %d > %d", new_value_len,
             sizeof(ctx->uri) - 1);
...
void golioth_register_frames(golioth_frame_cb_t cb)
{
    ...
    golioth_client_register_event_callback(golioth_frame_ctx.client,
                                           on_client_event, NULL);

    struct golioth_settings *settings =
        golioth_settings_init(golioth_frame_ctx.client);

    golioth_settings_register_string(settings, "FRAME", on_new_frame_setting,
                                     &golioth_frame_ctx);
