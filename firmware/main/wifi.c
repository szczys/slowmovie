...
#define CONFIG_ESP_WIFI_PW_ID "Home"
#define ESP_WIFI_AUTH_WPA2_PSK "WPA2 PSK"
...
void wifi_init_sta(struct credential *ssid, struct credential *psk)
{
    ...
    {
        ESP_LOGI(TAG, "connected to ap SSID:%s password:%s", ssid->buf, psk->buf);
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s", ssid->buf, psk->buf);
    }
    else
    {
